...
VM_OPTIONS_FILES_0=$IDE_BIN_HOME/studio$BITS.vmoptions
VM_OPTIONS_FILES_1=$HOME/.AndroidStudio/studio$BITS.vmoptions
VM_OPTIONS_FILES_2=$STUDIO_VM_OPTIONS
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablej2ee,disablealloc,delay=10000,sessionname=AndroidStudio"
  fi
fi

COMMON_JVM_ARGS="\"-Xbootclasspath/a:$IDE_HOME/lib/boot.jar\" -Didea.paths.selector=AndroidStudio $IDE_PROPERTIES_PROPERTY"
